...
from verl.utils.mindspore.pipeline_parallel import make_batch_generator
...
class MegatronRewardModel(BasePPORewardModel):
    ...
    def re_encode_by_rm_tokenizer(self, data: DataProto) -> DataProto:
        ...
        for id, mask in zip(input_ids, attention_mask, strict=True):
            # 1. remove pad for each sequence
            non_zero_indices = TorchAdapter.nonzero(mask).view(-1)
            ...
            if print_decode and TorchAdapter.distributed_get_rank() == 0:
            ...
            # 4. generate attention_mask and position_ids
            rm_attention_mask = TorchAdapter.ones_like(rm_input_ids, device=input_ids.device)
            ...
            rm_position_ids = TorchAdapter.arange(0, ori_seqlen, device=input_ids.device)
            input_ids_for_rm.append(rm_input_ids.unsqueeze(dim=0))
            attention_mask_for_rm.append(rm_attention_mask.unsqueeze(dim=0))
            position_ids_for_rm.append(rm_position_ids.unsqueeze(dim=0))
        input_ids_for_rm = TorchAdapter.cat(input_ids_for_rm, dim=0)
        attention_mask_for_rm = TorchAdapter.cat(attention_mask_for_rm, dim=0)
        position_ids_for_rm = TorchAdapter.cat(position_ids_for_rm, dim=0)
    ...
    def compute_reward(self, data: DataProto) -> DataProto:
        ...
        with TorchAdapter.no_grad():
            ...
            if MegatronAdapter.is_pipeline_last_stage(ignore_virtual=True):
                logits = TorchAdapter.cat(output["output"], dim=0)
                if use_dynamic_bsz:
                ...
                logits = TorchAdapter.empty(
                    (input_ids.shape[0], input_ids.shape[1]),
                    device=input_ids.device,
                )
            logits = logits.to(TorchAdapter.float32())
        ...
        ends = TorchAdapter.argmax(TorchAdapter.cumsum(attention_mask, dim=-1), dim=-1).view(-1, 1)  # (bs, 1)
        rewards = TorchAdapter.gather(token_level_rewards, dim=1, index=ends)  # (bs, 1)
        ...
        eos_mask_idx = TorchAdapter.argmax(position_ids * attention_mask, dim=-1)  # (bs,)
        eos_mask = TorchAdapter.zeros_like(attention_mask)
        eos_mask[TorchAdapter.arange(batch_size), eos_mask_idx] = 1.0
    ...
    def forward_batch(self, data: DataProto, use_dynamic_bsz=False, micro_batch_size=None, max_token_len=None):
        ...
        def loss_func(output):
            return TorchAdapter.tensor(1.0, device=output.device), output
        ...
        def forward_step(batch_iter, model):
            ...
            forward_fn = MegatronAdapter.get_mcore_forward_fn(self.hf_config)
