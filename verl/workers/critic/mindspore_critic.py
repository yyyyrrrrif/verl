...
class MegatronPPOCritic(BasePPOCritic):
    ...
    def compute_values(self, data: DataProto) -> DataProto:
        ...
        with TorchAdapter.no_grad():
            ...
            if MegatronAdapter.is_pipeline_last_stage(ignore_virtual=True):
                ...
                values = TorchAdapter.cat(values, dim=0).to(TorchAdapter.float32())
                if use_dynamic_bsz:
                ...
                values = TorchAdapter.empty_like(attention_mask)
                values = values.to(TorchAdapter.float32())
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if forward_only:
                return TorchAdapter.tensor(1.0, device=output.device), {"vpreds": output}
