...
from verl.utils.mindspore_adapter import MegatronAdapter, TorchAdapter
...
from verl.utils.checkpoint.mindspore_checkpoint_manager import MindSporeCheckpointManager
...
from verl.workers.actor.mindspore_actor import MegatronPPOActor
from verl.workers.config import HFModelConfig, McoreCriticConfig, RolloutConfig
from verl.workers.critic.mindspore_critic import MegatronPPOCritic
from verl.workers.reward_model.mindspore.reward_model import MegatronRewardModel
...
class MegatronWorker(Worker):
    def _init_hf_config_and_tf_config(
        ...
        override_transformer_config = MegatronAdapter.mapping_string_to_attn_backend(override_transformer_config)
        ...
        if use_mbridge:
            ...
            tf_config = MegatronAdapter.hf_to_mcore_config(hf_config, dtype, **override_transformer_config)
            self.bridge = None
...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        from verl.utils.mindspore.optimizer import (
        ...
        if self._is_actor:
            optim_config_megatron = init_megatron_optim_config(optim_config, fp16=self.dtype == TorchAdapter.float16())
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        rollout_device_mesh = TorchAdapter.init_device_mesh(
        ...
        self.torch_random_states = TorchAdapter.get_rng_state()
        gen_dp_rank = rollout_device_mesh["dp"].get_local_rank()
        TorchAdapter.manual_seed_device(gen_dp_rank + 1000)  # make sure all tp ranks have the same random states
        self.gen_random_states = TorchAdapter.get_rng_state()
        TorchAdapter.set_rng_state(self.torch_random_states)
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            override_transformer_config = {}
        self.param_dtype = TorchAdapter.PrecisionType().to_dtype(self.config.actor.megatron.dtype)
        log_gpu_memory_usage("Before init actor model and optimizer", logger=logger)
        self.dtype = TorchAdapter.PrecisionType().to_dtype(self.param_dtype)
        if self._is_actor:
        ...
        if self._is_actor:
            ...
            self.checkpoint_mananager = MindSporeCheckpointManager(
                config=self.config,
    ...
    async def rollout_mode(self):
        ...
        self.torch_random_states = TorchAdapter.get_rng_state()
        TorchAdapter.set_rng_state(self.gen_random_states)
...
class CriticWorker(MegatronWorker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        self.param_dtype = TorchAdapter.PrecisionType().to_dtype(self.config.megatron.dtype)
        self.dtype = TorchAdapter.PrecisionType().to_dtype(self.param_dtype)
        (
            self.critic_module,
