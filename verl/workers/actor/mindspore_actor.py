...
from verl.utils.mindspore.tensor_parallel import vocab_parallel_entropy, vocab_parallel_log_probs_from_logits
from verl.utils.mindspore_utils import get_model_config
...
class MegatronPPOActor(BasePPOActor):
    ...
    def forward_backward_batch(
        ...
        mini_batch = data
        TorchAdapter.broadcast_dict_tensor(
            mini_batch.batch,
            src=MegatronAdapter.get_pipeline_model_parallel_last_rank(),
            group=MegatronAdapter.get_pipeline_model_parallel_group(),
        )
        ...
        def forward_step(batch_iter, model, return_schedule_plan: bool = False):
            ...
            if self.use_fused_kernels:
                forward_fn = MegatronAdapter.get_mcore_forward_fused_fn(self.hf_config)
                if return_schedule_plan:
                    forward_fn = MegatronAdapter.gptmodel_forward_1f1b_overlap
