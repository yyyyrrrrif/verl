...
from verl.utils.mindspore_adapter import TorchAdapter, MegatronAdapter
from transformers import PretrainedConfig

import verl.utils.mindspore.tensor_parallel as tp_utils
...
def get_model_config(model):
    return MegatronAdapter.get_attr_wrapped_model(model, "config", allow_none=False)
...
def get_model(
    ...
    if model_type is None:
        model_type = MegatronAdapter.ModelType().encoder_or_decoder
    # Build model.
    if (
        MegatronAdapter.get_pipeline_model_parallel_world_size() > 1
        and MegatronAdapter.get_virtual_pipeline_model_parallel_world_size() is not None
    ):
        assert model_type != MegatronAdapter.ModelType().encoder_and_decoder, (
            "Interleaved schedule not supported for model with both encoder and decoder"
        )
        model = []
        has_vp_stage = inspect.signature(MegatronAdapter.is_pipeline_first_stage).parameters.get("vp_stage", None) is not None
        for i in range(MegatronAdapter.get_virtual_pipeline_model_parallel_world_size()):
            MegatronAdapter.set_virtual_pipeline_model_parallel_rank(i)
            # Set pre_process and post_process only after virtual rank is set.
            extra_kwargs = {} if not has_vp_stage else {"ignore_virtual": False, "vp_stage": i}
            pre_process = MegatronAdapter.is_pipeline_first_stage(**extra_kwargs)
            post_process = MegatronAdapter.is_pipeline_last_stage(**extra_kwargs)
            this_model = model_provider_func(pre_process=pre_process, post_process=post_process, vp_stage=i)
            this_model.model_type = model_type
            model.append(this_model)
        MegatronAdapter.set_virtual_pipeline_model_parallel_rank(0)
    else:
        pre_process = MegatronAdapter.is_pipeline_first_stage()
        post_process = MegatronAdapter.is_pipeline_last_stage()
        add_encoder = True
        add_decoder = True
        assert model_type != MegatronAdapter.ModelType().encoder_and_decoder, "Model type encoder_and_decoder is not supported"
        if model_type == MegatronAdapter.ModelType().encoder_and_decoder:
            if MegatronAdapter.get_pipeline_model_parallel_world_size() > 1:
                assert MegatronAdapter.get_pipeline_model_parallel_split_rank() is not None, (
                    "Split rank needs to be specified for model with both encoder and decoder"
                )
                rank = MegatronAdapter.get_pipeline_model_parallel_rank()
                split_rank = MegatronAdapter.get_pipeline_model_parallel_split_rank()
                world_size = MegatronAdapter.get_pipeline_model_parallel_world_size()
                pre_process = rank == 0 or rank == split_rank
                post_process = (rank == (split_rank - 1)) or (rank == (world_size - 1))
                add_encoder = MegatronAdapter.is_pipeline_stage_before_split()
                add_decoder = MegatronAdapter.is_pipeline_stage_after_split()
    ...
    for model_module in model:
        for param in model_module.parameters():
            MegatronAdapter.set_defaults_if_not_set_tensor_model_parallel_attributes(param)

    # Print number of parameters.
    if MegatronAdapter.get_data_parallel_rank() == 0:
        print(
            " > number of parameters on (tensor, pipeline) model parallel rank ({}, {}): {}".format(
                MegatronAdapter.get_tensor_model_parallel_rank(),
                MegatronAdapter.get_pipeline_model_parallel_rank(),
    ...
    if config.fp16 or config.bf16:  # the ModelParallelConfig in GPTModel
        model = [MegatronAdapter.Float16Module(config, model_module) for model_module in model]
    ...
    if wrap_with_ddp:
        ...
        ddp_config = MegatronAdapter.DistributedDataParallelConfig(**ddp_config_dict)
        for model_chunk_idx, model_chunk in enumerate(model):
            ddp_model = MegatronAdapter.DistributedDataParallel(
                config=tfconfig,
                module=model_chunk,
...
def _get_float16_module_class():
    from megatron.core.transformer.module import Float16Module
    return Float16Module

ALL_MODULE_WRAPPER_CLASSNAMES = (_get_ddp_class(), _get_float16_module_class())
...
def convert_config(hf_config: PretrainedConfig, megatron_config):
    ...
    overlap_p2p_comm = (
        MegatronAdapter.get_virtual_pipeline_model_parallel_world_size() is not None
        and MegatronAdapter.get_virtual_pipeline_model_parallel_world_size() > 1
    )
    batch_p2p_comm = False
    transformer_config = MegatronAdapter.TransformerConfig(
        ...
        activation_func=TorchAdapter.nn_functional_silu,
        ...
        tensor_model_parallel_size=MegatronAdapter.get_tensor_model_parallel_world_size(),
        pipeline_model_parallel_size=MegatronAdapter.get_pipeline_model_parallel_world_size(),
        virtual_pipeline_model_parallel_size=MegatronAdapter.get_virtual_pipeline_model_parallel_world_size(),
        context_parallel_size=MegatronAdapter.get_context_parallel_world_size(),
        overlap_p2p_comm=overlap_p2p_comm,
        batch_p2p_comm=batch_p2p_comm,
        pipeline_dtype=dt,
        params_dtype=dt,
        sequence_parallel=MegatronAdapter.get_tensor_model_parallel_world_size() > 1,
        ...
        bf16=dt is TorchAdapter.bfloat16(),
    )
...
def mcore_model_parallel_config(
    ...
    return MegatronAdapter.ModelParallelConfig(
        tensor_model_parallel_size=MegatronAdapter.get_tensor_model_parallel_world_size(),
        pipeline_model_parallel_size=MegatronAdapter.get_pipeline_model_parallel_world_size(),
        virtual_pipeline_model_parallel_size=MegatronAdapter.get_virtual_pipeline_model_parallel_world_size(),
        context_parallel_size=MegatronAdapter.get_context_parallel_world_size(),
...
def offload_megatron_model_to_cpu(models):
    ...
    for model_chunk in models:
        if isinstance(model_chunk, MegatronAdapter.DistributedDataParallel):
...
def offload_megatron_copy_params(optimizers):
    ...
    def _iter_opts(opt):
        if isinstance(opt, MegatronAdapter.ChainedOptimizer):
...
def convert_megatron_model_to_transformers_model(
    ...
    def convert_qkv_shard(full_tensor, q_name, k_name, v_name):
        ...
        new_params[q_name] = TorchAdapter.cat(q_shard_list, dim=0)
        new_params[k_name] = TorchAdapter.cat(k_shard_list, dim=0)
        new_params[v_name] = TorchAdapter.cat(v_shard_list, dim=0)
    ...
    def convert_gate_up_shard(full_tensor, gate_name, up_name):
        ...
        new_params[gate_name] = TorchAdapter.cat(gate_weight_list, dim=0)
        new_params[up_name] = TorchAdapter.cat(up_weight_list, dim=0)
...
def broadcast_from_megatron_pp(tensor):
    ...
    if tensor is not None:
        ...
        tensor_spec = None
    tensor_spec_output = [None] * MegatronAdapter.get_pipeline_model_parallel_world_size()
    TorchAdapter.distributed_all_gather_object(
        object_list=tensor_spec_output, obj=tensor_spec, group=MegatronAdapter.get_pipeline_model_parallel_group()
    )
    # find the src rank
    ...
    if tensor is None:
        tensor = TorchAdapter.empty(size=target_tensor_spec[0], dtype=target_tensor_spec[1], device=get_device_id())
    ...
    global_rank = TorchAdapter.distributed_get_global_rank(group=MegatronAdapter.get_pipeline_model_parallel_group(), group_rank=src_rank)
    TorchAdapter.distributed_broadcast(tensor=tensor, src=global_rank, group=MegatronAdapter.get_pipeline_model_parallel_group())
    return tensor


def broadcast_str_from_megatron_pp(obj: Any):
    obj_output = [None] * MegatronAdapter.get_pipeline_model_parallel_world_size()
    TorchAdapter.distributed_all_gather_object(object_list=obj_output, obj=obj, group=MegatronAdapter.get_pipeline_model_parallel_group())
    ...
    obj_output = [None] * TorchAdapter.distributed_get_world_size(group=MegatronAdapter.get_pipeline_model_parallel_group())
    obj_output[0] = target_obj
    TorchAdapter.distributed_broadcast_object_list(
        object_list=obj_output, src=global_rank, group=MegatronAdapter.get_pipeline_model_parallel_group()
    )
...
def default_tp_concat_fn(
    ...
    """
    train_tp_size = MegatronAdapter.get_tensor_model_parallel_world_size()
    if layer_name_mapping.get("qkv_layer_name") in name and "layer_norm" not in name:
        ...
        for infer_param in infer_params:
            ...
            for chunk in infer_param.chunk(num_query_groups_per_partition):
                ...
                k_lst.append(k)
                v_lst.append(v)
        q = TorchAdapter.cat(q_lst, dim=0)
        k = TorchAdapter.cat(k_lst, dim=0)
        v = TorchAdapter.cat(v_lst, dim=0)
        infer_params = TorchAdapter.cat((q, k, v), dim=0) if not convert_qkv_gate_up_by_simple_split else [q, k, v]
        ...
        for infer_param in infer_params:
            ...
            up_lst.append(up)
        gate = TorchAdapter.cat(gate_lst, dim=0)
        up = TorchAdapter.cat(up_lst, dim=0)
        infer_params = TorchAdapter.cat((gate, up), dim=0) if not convert_qkv_gate_up_by_simple_split else [gate, up]

    elif "mlp.experts.linear_fc2.weight" in name:  # moe
        infer_params = TorchAdapter.cat(infer_params, dim=1)

    else:
        # concat tensor
        infer_params = TorchAdapter.cat(infer_params, dim=tp_utils.get_tensor_parallel_partition_dim(train_params))
...
def per_tensor_generator(
    ...
    ep_size = MegatronAdapter.get_expert_model_parallel_world_size()
    etp_size = MegatronAdapter.get_expert_tensor_parallel_world_size()
    ep_group = MegatronAdapter.get_expert_model_parallel_group()
    etp_group = MegatronAdapter.get_expert_tensor_parallel_group()
    vpp_size = len(actor_module)
    all_gather_group = MegatronAdapter.get_tensor_model_parallel_group()
    all_gather_group_size = TorchAdapter.distributed_get_world_size(group=all_gather_group)
    ...
    obj_spec_output = [None] * MegatronAdapter.get_pipeline_model_parallel_world_size()
    TorchAdapter.distributed_all_gather_object(
        object_list=obj_spec_output, obj=meta_info, group=MegatronAdapter.get_pipeline_model_parallel_group()
    )
    ...
    for cur_pp_rank, scan_vpp_idx, idx, name in layer_list_meta:
        ...
        if ".mlp.experts.linear_fc" in cur_name and ep_size > 1:
            ...
            infer_params = [TorchAdapter.empty_like(broad_pp_tensor) for _ in range(ep_size)]
            TorchAdapter.distributed_all_gather(infer_params, broad_pp_tensor, group=ep_group)
            ...
            for name, param in zip(global_expert_names, infer_params, strict=True):
                if etp_size > 1:
                    # gather etp
                    etp_params = [TorchAdapter.empty_like(param) for _ in range(etp_size)]
                    TorchAdapter.distributed_all_gather(etp_params, param, group=etp_group)
                    params = etp_params
                else:
                    params = [param]
        ...
        if tp_utils.is_tensor_parallel_param(broad_pp_tensor):
            ...
            if all_gather_group_size <= 1:
                ...
                infer_params = [TorchAdapter.empty_like(broad_pp_tensor) for _ in range(all_gather_group_size)]
                TorchAdapter.distributed_all_gather(infer_params, broad_pp_tensor, group=MegatronAdapter.get_tensor_model_parallel_group())
...
def get_transformer_layer_offset(pipeline_rank, vp_stage, config):
    """
    ...
    has_vp_stage = (
        inspect.signature(MegatronAdapter.is_pipeline_first_stage).parameters.get("vp_stage", None) is not None
    )
    ...
    if config.pipeline_model_parallel_size > 1:
        if hasattr(config, "pipeline_model_parallel_layout") and config.pipeline_model_parallel_layout:
            ...
            if (vp_size := config.virtual_pipeline_model_parallel_size) is not None:
                ...
                if config.account_for_embedding_in_pipeline_split and not MegatronAdapter.is_pipeline_first_stage(
                    **extra_kwargs
                ):
                    offset -= 1
            else:
...
def register_megatron_training_hooks(model, optimizer):
    try:
        from megatron.core.distributed.fsdp.mcore_fsdp_adapter import FullyShardedDataParallel as megatron_FSDP
    except ImportError:
        from megatron.core.distributed import DistributedDataParallel as megatron_FSDP
    ...
    for one_model in model:
        config = MegatronAdapter.get_attr_wrapped_model(one_model, "config", allow_none=False)
        ...
        if (isinstance(model[0], megatron_FSDP) or isinstance(model[0], MegatronAdapter.DistributedDataParallel)) and overlap_grad_reduce:
