...
from verl.utils.mindspore.dist_checkpointing import load_dist_checkpointing, save_dist_checkpointing
from verl.utils.mindspore_adapter import TorchAdapter, MegatronAdapter
from verl.utils.mindspore_utils import (
...
class MindSporeCheckpointManager(BaseCheckpointManager):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bridge = bridge
        self.rank = TorchAdapter.distributed_get_rank()
    ...
    def get_rng_state(self, use_dist_ckpt: bool = True, data_parallel_random_init: bool = False):
        ...
        rng_state = {
            ...
            "torch_rng_state": TorchAdapter.get_rng_state_torch(),
            "rng_tracker_states": MegatronAdapter.get_cuda_rng_tracker().get_states(),
        }
        ...
        rng_state_list = None
        if TorchAdapter.distributed_is_initialized() and MegatronAdapter.get_data_parallel_world_size() > 1 and data_parallel_random_init:
            rng_state_list = [None for i in range(MegatronAdapter.get_data_parallel_world_size())]
            TorchAdapter.distributed_all_gather_object(rng_state_list, rng_state, group=MegatronAdapter.get_data_parallel_group())
        else:
            rng_state_list = [rng_state]

        if use_dist_ckpt:
            pp_rank = MegatronAdapter.get_pipeline_model_parallel_rank()
            pp_size = MegatronAdapter.get_pipeline_model_parallel_world_size()
            tp_rank = MegatronAdapter.get_tensor_model_parallel_rank()
            tp_size = MegatronAdapter.get_tensor_model_parallel_world_size()
            rng_state_list = MegatronAdapter.ShardedObject(
                "rng_state",
                rng_state_list,
                (pp_size, tp_size),
                (pp_rank, tp_rank),
                replica_id=MegatronAdapter.get_data_parallel_rank(with_context_parallel=True),
            )
    ...
    def get_checkpoint_name(
        ...
        if pipeline_parallel is None:
            pipeline_parallel = MegatronAdapter.get_pipeline_model_parallel_world_size() > 1
        if tensor_rank is None:
            tensor_rank = MegatronAdapter.get_tensor_model_parallel_rank()
        if pipeline_rank is None:
            pipeline_rank = MegatronAdapter.get_pipeline_model_parallel_rank()
        if cp_rank is None:
            cp_rank = MegatronAdapter.get_context_parallel_rank()
        if expert_parallel is None:
            expert_parallel = MegatronAdapter.get_expert_model_parallel_world_size() > 1
        if expert_rank is None:
            expert_rank = MegatronAdapter.get_expert_model_parallel_rank()
    ...
    def generate_state_dict(
        ...
        for vpp_rank, model in enumerate(self.model):
            if len(self.model) > 1:
                MegatronAdapter.set_virtual_pipeline_model_parallel_rank(vpp_rank)
    ...
    def load_rng_states(self, rng_states, data_parallel_random_init=False, use_dist_ckpt=True):
        ...
        if data_parallel_random_init:
            rng_states = rng_states[MegatronAdapter.get_data_parallel_rank()]
        else:
            rng_states = rng_states[0]
        random.setstate(rng_states["random_rng_state"])
        np.random.set_state(rng_states["np_rng_state"])
        TorchAdapter.set_rng_state_torch(rng_states["torch_rng_state"])
        ...
        if not rng_states["rng_tracker_states"]:
            raise KeyError
        MegatronAdapter.get_cuda_rng_tracker().set_states(rng_states["rng_tracker_states"])
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        TorchAdapter.serialization_add_safe_globals([torch.optim.AdamW])
        TorchAdapter.serialization_add_safe_globals([transformer_engine.pytorch.optimizers.fused_adam.FusedAdam])
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.should_save_extra:
            if self.rank == 0:
                ...
                import torch
                to_convert_types = {torch.dtype: str, MegatronAdapter.AttnBackend(): str}
