...
def get_default_kwargs_for_model_parallel_config():
    model_parallel_config_kwargs = {
        "params_dtype": TorchAdapter.float32(),
...
def get_default_model_parallel_config():
    return MegatronAdapter.ModelParallelConfig(**get_default_kwargs_for_model_parallel_config())
...
def get_default_kwargs_for_column_parallel_linear():
    ...
    column_default_kwargs = {
        "config": MegatronAdapter.ModelParallelConfig(**model_parallel_config_kwargs),
    }
...
class _VocabParallelEntropy(TorchAdapter.autograd_Function()):
    @staticmethod
    def forward(ctx, vocab_parallel_logits):
        # Use torch.compile through adapter
        compiled_mul_reduce = TorchAdapter.compile(dynamic=True)(lambda a, b: (a * b).sum(dim=-1, keepdim=True))
        
        def mul_reduce(a, b):
            return compiled_mul_reduce(a, b)

        logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
        ReduceOp = TorchAdapter.distributed_ReduceOp()
        TorchAdapter.distributed_all_reduce(logits_max, op=ReduceOp.MAX, group=MegatronAdapter.get_tensor_model_parallel_group())
        normalized_vocab_parallel_logits = vocab_parallel_logits - logits_max
        normalized_exp_logits = normalized_vocab_parallel_logits.exp_()
        normalized_sum_exp_logits = normalized_exp_logits.sum(dim=-1, keepdim=True)
        TorchAdapter.distributed_all_reduce(normalized_sum_exp_logits, group=MegatronAdapter.get_tensor_model_parallel_group())
        softmax_logits = normalized_exp_logits.div_(normalized_sum_exp_logits)
        sum_softmax_times_logits = mul_reduce(softmax_logits, vocab_parallel_logits)
        TorchAdapter.distributed_all_reduce(sum_softmax_times_logits, group=MegatronAdapter.get_tensor_model_parallel_group())
...
def vocab_parallel_entropy(vocab_parallel_logits):
...
def vocab_parallel_log_probs_from_logits(logits, labels):
    ...
    return -MegatronAdapter.vocab_parallel_cross_entropy(vocab_parallel_logits=logits, target=labels)
...
def vocab_parallel_log_probs_from_logits_response_rmpad(input_ids, attention_mask, logits_rmpad, response_length):
    ...
    input_ids_rmpad_rolled = TorchAdapter.roll(input_ids_rmpad, shifts=-1, dims=0)
