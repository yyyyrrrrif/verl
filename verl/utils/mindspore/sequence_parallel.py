...
def pad_to_sequence_parallel(unpad_tokens):
    ...
    sp_world_size = MegatronAdapter.get_tensor_model_parallel_world_size()

    pad_size = 0 if total_nnz % sp_world_size == 0 else sp_world_size - total_nnz % sp_world_size

    if pad_size > 0:
        if unpad_tokens.ndim == 1:
            unpad_tokens = TorchAdapter.nn_functional_pad(unpad_tokens, (0, pad_size))
        elif unpad_tokens.ndim == 2:
            unpad_tokens = TorchAdapter.nn_functional_pad(unpad_tokens, (0, 0, 0, pad_size))
        else:
