...
class _VocabParallelEntropy(TorchAdapter.autograd_Function()):
    ...
    def forward(ctx, vocab_parallel_logits):
        @TorchAdapter.compile(dynamic=True)
        def mul_reduce(a, b):
            return (a * b).sum(dim=-1, keepdim=True)

        logits_max = vocab_parallel_logits.max(dim=-1, keepdim=True).values
        reduce_op = TorchAdapter.distributed_ReduceOp()
        TorchAdapter.distributed_all_reduce(
            logits_max, 
            op=reduce_op.MAX, 
            group=MegatronAdapter.get_tensor_model_parallel_group()
        )
        normalized_vocab_parallel_logits = vocab_parallel_logits - logits_max
        normalized_exp_logits = normalized_vocab_parallel_logits.exp_()
        normalized_sum_exp_logits = normalized_exp_logits.sum(dim=-1, keepdim=True)
        TorchAdapter.distributed_all_reduce(
            normalized_sum_exp_logits, 
            group=MegatronAdapter.get_tensor_model_parallel_group()
        )
